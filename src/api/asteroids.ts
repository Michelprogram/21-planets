    ...
    request.data.near_earth_objects.forEach((element: any) => {
      let tmp: IAsteroide = {
        ...
        asteroide: {
          ...
          distance_from_earth:
            Math.round(
              parseInt(element.close_approach_data[0].miss_distance.kilometers)
            ) + "",
          velocity: Math.round(
            element.close_approach_data[0].relative_velocity.kilometers_per_hour
          ),
          superficie:
            ((3.14 * 4 * (randomNotFloor(2000, 3000) / 2)) ^ 2) * 1000,
          icon: randomIcon(),
          color: color,
        },
      };
